...
void Cloth::initializeSpring() {
  ...
  // connects vertical structrual springs
  for (int i = 0; i < particlesPerEdge - 1; ++i) {
    for (int j = 0; j < particlesPerEdge; ++j) {
      int index = i * particlesPerEdge + j;
  ...
  // connects shear springs from left-top to right-bottom
  ...
  // connects shear springs from right-top to left-bottom
  for (int i = 0; i < particlesPerEdge - 1; ++i) {
  ...
  // connects horizontal stretch springs
  float stretchLength = structrualLength * 2;
  for (int i = 0; i < particlesPerEdge; ++i) {
    for (int j = 0; j < particlesPerEdge - 2; ++j) {
      int index = i * particlesPerEdge + j;
      _springs.emplace_back(index, index + 2, stretchLength, Spring::Type::BEND);
    }
  }

  // connects vertical stretch springs
  for (int i = 0; i < particlesPerEdge - 2; ++i) {
    for (int j = 0; j < particlesPerEdge; ++j) {
      int index = i * particlesPerEdge + j;
      _springs.emplace_back(index, index + particlesPerEdge * 2, stretchLength, Spring::Type::BEND);
    }
  }
...
void Cloth::computeSpringForce() {
  ...
  for (auto& i : _springs) {
    auto start = i.startParticleIndex(), end = i.endParticleIndex();
    auto displacement = _particles.position(end) - _particles.position(start);
    auto unitDisplacement = displacement.normalized();

    auto springForce = -springCoef * (displacement.norm() - i.length()) * unitDisplacement;
    _particles.acceleration(start) -= springForce * _particles.inverseMass(start);
    _particles.acceleration(end) += springForce * _particles.inverseMass(end);

    auto velocityDifference = _particles.velocity(end) - _particles.velocity(start);
    auto damperForce = -damperCoef * unitDisplacement.dot(velocityDifference) * unitDisplacement;
    _particles.acceleration(start) -= damperForce * _particles.inverseMass(start);
    _particles.acceleration(end) += damperForce * _particles.inverseMass(end);
  }
}
