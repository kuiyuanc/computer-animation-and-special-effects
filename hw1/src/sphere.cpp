...
void Spheres::collide(Cloth* cloth) {
  ...
  constexpr float tolerance = 1 + 15e-3;
  for (auto i = 0; i < sphereCount; ++i) {
    for (auto j = 0; j < particlesPerEdge * particlesPerEdge; ++j) {
      auto N = cloth->particles().position(j) - _particles.position(0);

      if (N.norm() > _radius[i] * tolerance) continue;
      if ((j + 1) % particlesPerEdge != 0) {
        cloth->particles().position(j) = _particles.position(0) + N.normalized() * (_radius[i] * tolerance);
      }

      if (cloth->particles().velocity(j).dot(N) > 0) continue;
      cloth->particles().velocity(j) += -N.normalized() * cloth->particles().velocity(j).dot(N.normalized()) * 2;
    }
  }
}
